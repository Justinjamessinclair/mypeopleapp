...
function styleRecenterBtn(geoBtn) {
  // Get computed styles from geolocate button
  const geoStyle = window.getComputedStyle(geoBtn);
  recenterBtn.style.display = geoStyle.display;
  ...
  recenterBtn.style.width = geoStyle.width;
  recenterBtn.style.height = geoStyle.height;
  recenterBtn.style.borderRadius = geoStyle.borderRadius;
  recenterBtn.style.background = geoStyle.background;
  recenterBtn.style.boxShadow = geoStyle.boxShadow;
  recenterBtn.style.border = geoStyle.border;
  recenterBtn.style.padding = geoStyle.padding;
  recenterBtn.style.alignItems = geoStyle.alignItems;
  recenterBtn.style.justifyContent = geoStyle.justifyContent;
  recenterBtn.style.fontSize = geoStyle.fontSize;
  recenterBtn.style.cursor = geoStyle.cursor;
  recenterBtn.style.outline = geoStyle.outline;
  recenterBtn.style.transition = geoStyle.transition;
  recenterBtn.style.color = geoStyle.color;
  recenterBtn.style.backgroundColor = geoStyle.backgroundColor;
  recenterBtn.style.minWidth = geoStyle.minWidth;
  recenterBtn.style.minHeight = geoStyle.minHeight;
  recenterBtn.style.zIndex = geoStyle.zIndex;

  // Insert after geolocate button
  geoBtn.parentNode.insertBefore(recenterBtn, geoBtn.nextSibling);

  // Use a similar icon
  recenterBtn.innerHTML = '<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="11" cy="11" r="7" stroke="currentColor" stroke-width="2"/><circle cx="11" cy="11" r="2" fill="currentColor"/></svg>';
}
...
sheetViewButton.addEventListener('click', () => {
  ...
  if (isSheetView) {
    ...
    sheetViewButton.textContent = 'sheet';
    if (miniMapPopup) miniMapPopup.style.display = 'none';
  }
});